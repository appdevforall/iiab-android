...
> **Current status (2026-06-25):** Phase 0 (guardrails) ✅ and Phase 1 (security) ✅ complete (core). 28 JVM unit-test files; 21 clean-architecture slices; lint is a hard gate. **Phase 3 — major progress:** `D1` (`DeployFragment`) **DONE** — carved 3,032 → ~855 LOC across backup / planner / install / reset-delete / ADB-share (ADFA-4434, ADFA-4440, ADFA-4441), and the **3 Fragment lint detectors are RE-ENABLED** (ADFA-4443) — the workaround below is **retired**. `MissingTranslation` is back to a hard error (ADFA-4453). **Still open — Phase 2 (concurrency), untouched:** ~37 raw `new Thread(` across 17 files (only 3 use an `Executor`); `M1` (wakelock, no timeout) and `F12` (`MODE_MULTI_PROCESS`) still present. **Phase 3 remainder:** `F1` (`MainActivity` ~2,384 LOC) and `S14` (`SyncFragment` ~811) not carved; `D3` (central config/endpoints) not done. **Phase 4:** `M15` done; `M8` (jcenter ×2), `M9` (targetSdk 28 — blocks Play Store), `minifyEnabled false`, and `M19` (`DEBUG_ENABLED` hardcoded) still open. Sections 1–6 below are the original 2026-06-16 audit snapshot.
