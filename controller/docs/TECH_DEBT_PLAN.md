...
_Last updated: 2026-06-30. Tracks remediation work against the findings below. IDs map to the register in this file (F/D/S/M) and to `FORK_DELTA_ANALYSIS.md` (K)._

> **Current status (2026-06-30):** **Phase 3 — Share tab carved: `S14` DONE (epic ADFA-1028, umbrella ADFA-4492).** `SyncFragment`'s mechanic was extracted into a clean, exportable stack: a pure `sync/domain` (rsyncd.conf/argv builders, progress parse, exit-code→outcome, `ShareConfig`, `TransferGuard`, `SyncCredentialValidator` — no `android.*`, JVM-tested), a `TransportEngine` port with `RsyncManager` as its adapter, platform adapters (`NetworkInterfaces`/`QrCodec`/`SecretStore`), and an Activity-scoped `SyncStateViewModel` + `SyncProgressRepository` so the probe/dry-run/transfer survive a configuration-change recreation (PRs #93/#94/#96/#98/#99/#104). Folded in and closed here: `S7` (hardcoded ports → `ShareConfig`), `S8`/`S9`/`S10` (concurrency/lifecycle), `S11` (rsync secret lifecycle), `S15`/`S16` (theming / magic-strings), the `EX1–EX6` export items, and **`D17`** (`ApkServer` now GET-only). **Residual:** the `SyncFragment` view itself is still ~890 LOC (the carve extracted the mechanic/state but left a fat orchestrator) → carve into `SyncViewModel` + per-area controllers tracked as **ADFA-4506** (Low, non-blocking; the CoGo export does not depend on it). **Separately, real-world Share robustness (bug ADFA-4496, closed):** Wi-Fi network binding for the receive (#105), IP-under-QR transparency, a phantom-process SIGKILL (exit 137) safety net (#107), and an informed pre-flight + connection-failed hint (#109); the recurring "Connection Failed" was root-caused to **AP client-isolation** (a network condition), not the app. **Phase 3 remainder:** `F1` (`MainActivity` ~2,384 LOC) not carved; `D3` (central config/endpoints) not done.

> **Current status (2026-06-27):** Phase 0 (guardrails) ✅, Phase 1 (security) ✅, and **Phase 2 (concurrency) ✅ closed**. 28 JVM unit-test files; lint is a hard gate. **Phase 2 sweep (epic ADFA-1028, all four areas):** introduced a shared `util/AppExecutors` (pooled `io()` + `scheduler()`); Dashboard polling → lifecycle-scoped ViewModel off the main thread (ADFA-4457); Sync/ADB pairing-thread leak fixed, rsync daemon start moved off the UI thread, process handle made `volatile` (ADFA-4456); Deploy/PRoot `currentProcess` `volatile` + reaped on kill (ADFA-4459, D19); MainActivity one-off threads → executor and recurring `Handler` loops audited-clean (ADFA-4460). `D9` (`DeployFragment` `static` download state → Activity-scoped `DownloadStateViewModel`) **DONE** (ADFA-4459, PR #85). **Deferred Phase-2 residuals (low priority):** the long-lived proot worker threads; `F12` (`MODE_MULTI_PROCESS`) and the VPN dead-code removal also deferred. `M1` (WatchdogService wakelock) intentionally keeps no timeout (multi-hour transfers; released in `onDestroy`). **Phase 3 — major progress:** `D1` (`DeployFragment`) **DONE** — carved 3,032 → ~855 LOC across backup / planner / install / reset-delete / ADB-share (ADFA-4434, ADFA-4440, ADFA-4441), and the **3 Fragment lint detectors are RE-ENABLED** (ADFA-4443) — the workaround below is **retired**. `MissingTranslation` is back to a hard error (ADFA-4453). **Phase 3 — install survives recreation: DONE (ADFA-4474).** The rootfs install pipeline (download → extract → companion data: Kiwix + maps Ansible → finish) was moved into a dedicated **foreground `InstallService`** that is independent of the Fragment/Activity lifecycle, driven by an observable app-scoped `InstallProgressRepository`; the install now survives a configuration-change recreation (theme toggle), orientation changes and backgrounding, with a progress notification + Cancel. The planner selection (tier / companion-data / Kiwix overrides) moved to the Activity-scoped ViewModel so the **projection gauge + selections** re-render after recreation too (PRs #87 / #89 / #90; verified on-device). **Follow-up `ADFA-4476` (open):** module-grid checkbox selection survival, and routing the scratch-reset (clean Debian) download + the per-module installs through the same service/repository. **Phase 3 remainder:** `F1` (`MainActivity` ~2,384 LOC) and `S14` (`SyncFragment` ~811) not carved; `D3` (central config/endpoints) not done. **Phase 4:** `M15` done; `M8` (jcenter ×2), `M9` (targetSdk 28 — blocks Play Store), `minifyEnabled false`, and `M19` (`DEBUG_ENABLED` hardcoded) still open. Sections 1–6 below are the original 2026-06-16 audit snapshot.
